...
struct Myers64x2Input {
  ...
  int q_wrd_len;
  const char *d_wrds[2];
  uint32_t d_wrd_lens[2];
};

std::array<uint32_t, 16> levenshtein_myers_8x16(const Myers8x16Input &input);
...
std::array<uint32_t, 2> levenshtein_myers_64x2(const Myers64x2Input &input);
